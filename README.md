...
The `variable` and `extract-regexp` features work on the full request and response payload text, without breakdown into URI/status/headers/body. Note that variable and regexp usage _will_ make your tests to work a bit slower, due to the processing overhead. Also some more RAM will be used by the load generator.

For HTTP, there is special `:content-length:` variable to be used to obtain connect body length when variable usage modify it. Taurus module will automatically use that variable when generating POST requests.
